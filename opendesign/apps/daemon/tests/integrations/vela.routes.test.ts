...
describe('local-only cloud boundary', () => {
  it.each([
    ['GET', '/api/amr/models'],
    ...
    ['GET', '/api/integrations/vela/api-proxy/v1/models'],
  ])('returns 410 for %s %s', async (method, route) => {
    const response = await fetch(`${baseUrl}${route}`, { method });
    expect(response.status).toBe(410);
    await expect(response.json()).resolves.toMatchObject({
      error: 'open_design_cloud_disabled',
    });
  });
});

// Upstream cloud-contract coverage is retained for future rebase context, but
// it is intentionally inapplicable while the local-only boundary above is on.
describe.skip('GET /api/integrations/vela/wallet', () => {
...
describe.skip('GET /api/integrations/vela/status', () => {
...
describe.skip('POST /api/integrations/vela/login', () => {
...
describe.skip('ALL /api/integrations/vela/api-proxy/*', () => {
...
describe.skip('ALL /api/integrations/vela/message-center/*', () => {
...
describe.skip('POST /api/integrations/vela/logout', () => {
...
describe.skip('login → status round-trip (E2E across the three routes)', () => {
