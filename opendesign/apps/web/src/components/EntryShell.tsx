...
export function EntryShell({
  ...
  const railWorkspaceContext = accountFooterState === 'sign-in'
    ? null
    : workspaceContext;
  // Cloud identity never gates the local application in this fork.
...
function OnboardingView({
  ...
  const t = useT();
  const analytics = useAnalytics();
  // Start at execution-source selection; the upstream step 0 is mandatory
  // Open Design Cloud identity and is intentionally skipped in this fork.
  ...
  useEffect(() => {
    // Do not probe cloud login state during local-only onboarding.
