...
in
{
  ...
  package = lib.mkOption {
    ...
    default =
      flakePackages.opendesign
        or (throw "open-design: no daemon package available for ${system}; set services.open-design.package explicitly");
    defaultText = lib.literalExpression "good-vibes-only.packages.\${pkgs.stdenv.hostPlatform.system}.opendesign";
  ...
  port = lib.mkOption {
    ...
    description = ''
      ...
      `OD_DAEMON_URL` injection — see `nix/opendesign/README.md`.
    '';
  };
  ...
  webFrontend = {
    ...
    package = lib.mkOption {
      type = lib.types.package;
      default =
        flakePackages.opendesign-web
          or (throw "open-design: no web package available for ${system}; set services.open-design.webFrontend.package explicitly");
      defaultText = lib.literalExpression "good-vibes-only.packages.\${pkgs.stdenv.hostPlatform.system}.opendesign-web";
