# NixOS module for the officially released OpenDesign source.
...
# Manager module (nix/opendesign/home-manager.nix).
#
# Usage from this repository's flake:
#   imports = [ inputs.good-vibes-only.nixosModules.open-design ];
...
let
  ...
  # See nix/opendesign/home-manager.nix for the rationale behind these handle
  ...
  # assertion. See nix/opendesign/home-manager.nix for the rationale.
  isLoopbackHost =
  ...
  // lib.optionalAttrs cfg.webFrontend.enable {
    # See nix/opendesign/home-manager.nix — the daemon's /api origin allowlist
    ...
    lib.mkMerge [
      ...
      (lib.mkIf cfg.autoStart {
        systemd.services.open-design = {
          ...
          # NixOS synthesizes a default PATH for every systemd service. The
          # daemon needs its explicit discovery PATH instead, so give this one
          # value higher module priority while leaving the other environment
          # entries normally mergeable.
          environment = daemonEnvironment // {
            PATH = lib.mkForce daemonEnvironment.PATH;
          };
