...
export function registerVelaRoutes(app: Express, deps: RegisterVelaRoutesDeps): void {
  // This fork is deliberately local-only. Keep the cloud implementation in
  // the source tree to make upstream rebases reviewable, but put a hard stop
  // in front of every AMR/Open Design Cloud endpoint. This is intentionally a
  // daemon-side boundary rather than only a hidden UI control: old clients,
  // plugins, and direct HTTP callers must not be able to start device login or
  // send authenticated requests to the hosted service.
  const cloudDisabled = (_req: Request, res: Response): void => {
    res.status(410).json({
      error: 'open_design_cloud_disabled',
      message: 'Open Design Cloud is disabled in this local-only fork.',
    });
  };
  app.use('/api/amr', cloudDisabled);
  app.use('/api/integrations/vela', cloudDisabled);
