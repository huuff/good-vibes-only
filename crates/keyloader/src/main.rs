...
fn remaining_since(started_at: u64, lifetime: u64) -> u64 {
    started_at
        .saturating_add(lifetime)
        .saturating_sub(cache::now())
}
...
fn discover() -> Result<()> {
    ...
    let previous = cache::load().ok().flatten();
    let learned: HashMap<String, (Option<u64>, String)> = previous
        .as_ref()
        .map(|old| {
            old.gpg
                .iter()
                .filter_map(|item| {
                    Some((
                        ...
                        (item.summary.version, item.fingerprint.clone()?),
                    ))
                })
                .collect()
        })
    ...
    let mut refreshed = cache::Cache::new(ssh_items, gpg_items);
    if let Some(previous) = &previous {
        refreshed.carry_tracking_from(previous);
    }
    let path = cache::store(&refreshed)?;
    println!();
...
fn status() -> Result<()> {
    ...
    let ssh_retention = ssh::retention_policy();
            ...
            Some(fpr) if loaded.contains(fpr) => {
                ...
                match (
                    ...
                    (ssh::RetentionPolicy::Limited(lifetime), Some(loaded_at)) => {
                        let remaining = remaining_since(*loaded_at, *lifetime);
                        if remaining > 0 {
                            state.push_str(&format!(
                                ", ~{} remaining",
                                cache::human_duration(remaining)
                            ));
                        } else {
                            state.push_str(", remaining time unknown (tracked expiry passed)");
                        }
                    }
                    (ssh::RetentionPolicy::Forever, _) => {
                        state.push_str(", until ssh-agent exits");
                    }
                    _ => state.push_str(", remaining time unknown"),
                }
                ("✓".green(), state)
            }
            Some(_) => ("✗".yellow(), "not loaded".to_string()),
            None => (
                "?".yellow(),
                "unknown (item has no fingerprint field)".to_string(),
            ),
        };
    ...
    let gpg_max_ttl = gpg::max_cache_ttl().ok();
        ...
        let (glyph, state) = match item.fingerprint.as_deref() {
            ...
            Some(fpr) => match gpg::secret_key(fpr)? {
                ...
                Some(key) => {
                    ...
                    let mut reported_remaining = false;
                    if let (Some(max_ttl), Some(tracked)) = (
                        gpg_max_ttl,
                        cached_items.gpg_preset_at.get(&item.summary.id),
                    ) {
                        let remaining: Vec<u64> = key
                            .keygrips
                            .iter()
                            .filter(|grip| cached_grips.contains(*grip))
                            .filter_map(|grip| tracked.get(grip))
                            .map(|preset_at| remaining_since(*preset_at, max_ttl))
                            .filter(|remaining| *remaining > 0)
                            .collect();
                        if let Some(next) = remaining.iter().min() {
                            reported_remaining = true;
                            state.push_str(&format!(
                                ", next expiry in ~{}",
                                cache::human_duration(*next)
                            ));
                            if remaining.len() < hot {
                                state.push_str(" (other expiry times unknown)");
                            }
                        }
                    }
                    if hot > 0 && !reported_remaining {
                        state.push_str(", remaining time unknown");
                    }
                    (glyph, state)
                }
            },
        ...
        match load_gpg_item(item, &cached_grips, dry_run) {
            Ok((msg, preset_grips)) => {
                if !preset_grips.is_empty() {
                    ...
                    let tracked = cached_items
                        .gpg_preset_at
                        .entry(item.summary.id.clone())
                        .or_default();
                    for grip in preset_grips {
                        tracked.insert(grip, preset_at);
                    }
                    changed = true;
                }
                println!(
